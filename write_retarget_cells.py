...
mobiles = json.load(open("data/zi_retarget_mobiles_found.json"))
cells = []
for m in mobiles:
    rid = m["row_id"]
    ...
    r = row_by_id[rid]
    res = m["result"]
    cells.append(gspread.Cell(r, idx["Owner Cell"] + 1, res["mobilePhone"]))
...
print(f"Wrote cells for {len(mobiles)} newly-found mobiles ({len(cells)} total cell updates)")
